...
export interface WhisperModel {
  ...
  capabilities: {
    ...
    quantizable: boolean;
    tdrz?: boolean; // opcional TDRZ capability para modelos nativos
  };
}
...
export function useWhisperModels() {
  ...
  const getModelDirectory = useCallback(async () => {
    const dirPath = `${RNFS.DocumentDirectoryPath}/whisper-models`;
    try {
  ...
  const downloadModel = useCallback(
    async (model: WhisperModel) => {
      ...
      // Helper to update \\`modelFiles\\` state after download or if file exists
      const updateModelFileInfo = async () => {
        try {
          ...
          setModelFiles((prev) => ({
            ...prev,
            [model.id]: {
              path: `file://${filePath}`, // whisper.rn expects a file URI
  ...
  const initializeWhisperModel = useCallback(
    async (modelId: string, options?: { initVad?: boolean }) => {
      ...
      try {
        ...
        // Descargar modelo si no está ya disponible
        ...
        // Inicializar opcionalmente el contexto VAD
        if (options?.initVad) {
  ...
  const deleteModel = useCallback(
    async (modelId: string) => {
      ...
      // Eliminar prefijo file:// para RNFS
      const filePath = fileInfo.path.replace("file://", "");

      try {
        if (await RNFS.exists(filePath)) {
          await RNFS.unlink(filePath);
          console.log(`Deleted model file at ${filePath}`);
        }
      } catch (error) {
